...
Record floatInfo := {
  isHex_FI:bool;
  integer_FI:option str;
  fraction_FI:option str;
  exponent_FI:option str;
  suffix_FI:option str
}.
...
Inductive typeSpecifier := (* Merge all specifiers into one type *)
  ...
  | Tstruct_union : structOrUnion -> option str -> option (list field_group) -> list attribute -> typeSpecifier
  | Tenum : option str -> option (list (str * option expression * loc)) -> list attribute -> typeSpecifier
...
(* The bool is true for variable length parameters. *)
 ...
 | PROTO_OLD : decl_type -> list str -> decl_type

with parameter :=
  | PARAM : list spec_elem -> option str -> decl_type -> list attribute -> loc -> parameter
...
(* the str, and decl_type will be PTR([], JUSTBASE) *)
with name :=
  | Name : str -> decl_type -> list attribute -> loc -> name
...
with expression :=
  ...
  | MEMBEROF : expression -> str -> expression
  | MEMBEROFPTR : expression -> str -> expression
...
with constant :=
  (* The str is the textual representation of the constant in
     the source code. *)
  | CONST_INT : str -> constant
  | CONST_FLOAT : floatInfo -> constant
  | CONST_CHAR : encoding -> list char_code -> constant
  | CONST_str : encoding -> list char_code -> constant
...
with gcc_attribute_word :=
  | GCC_ATTR_IDENT : str -> gcc_attribute_word
  | GCC_ATTR_CONST
  | GCC_ATTR_PACKED.
...
| ASMOPERAND: option str -> encoding -> list char_code -> expression -> asm_operand.
...
with statement :=
 ...
 | LABEL : str -> statement -> loc -> statement
 | GOTO : str -> loc -> statement
