...
%token<Cabs.str * Cabs.loc> VAR_NAME TYPEDEF_NAME OTHER_NAME
%token<Cabs.str * Cabs.loc> PRAGMA
...
%type<list (Cabs.str * option Cabs.expression * Cabs.loc) (* Reverse order *)>
  enumerator_list
%type<Cabs.str * option Cabs.expression * Cabs.loc> enumerator
%type<Cabs.str * Cabs.loc> enumeration_constant
...
%type<list Cabs.str (* Reverse order *)> identifier_list
%type<list Cabs.asm_flag> asm_flags
%type<option Cabs.str> asm_op_name
...
| str = STRING_LITERAL
    { let '((enc, chars), loc) := str in
      (Cabs.CONSTANT (Cabs.CONST_str enc chars), loc) }
...
| loc = STATIC_ASSERT LPAREN expr = constant_expression
    ...
    { let '((enc, chars), locs) := str in
      (expr, (Cabs.CONST_str enc chars, locs), loc) }
